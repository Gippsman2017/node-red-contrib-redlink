...
module.exports.RedLinkStore = function (config) {
    ...
    function notifyPeerStoreOfConsumers(ip, port, ipTrail, notifyDirection) {
        ...
        if (!ipTrail) {  ipTrail = []; }  //loop in notifications- dont send it
        if (ipTrail.includes(ip + ':' + port)) {  return; } else {  ipTrail.push(ip + ':' + port); }
            ...
            if (newMessage) {
                ...
                remoteMatchingStores.forEach(remoteStore => {
                    ...
                    request(options, function (error, response) {
                        ...
                        { // Should really do something with the response .. John
    ...
    app.post('/notify', (req, res) => { //todo validation on params
        ...
        switch (notifyType) {
            ...
            case 'consumerRegistration' :
                ...
                if (notifyDirection === notifyDirections.SOUTH) {thisRegistrationDirection = 'north';};
...
//                       console.log(node.name,' - ',insertGlobalConsumersSql);
                ...
                if (thisRegistrationDirection == 'south'){ notifyNorthStoreOfConsumers(ips); }
            ...
            case 'producerNotification' :
                ...
                node.send([null, { storeName: node.name, action: 'forwardProducerNotification', direction: 'outBound', Data: {globalCons}}, null]);
...
//                       console.log(node.name,'      forward this notify to ',consumer.globalStoreName);
                       ...
                       node.send([null, { storeName: node.name, action: 'producerForwardNotification', direction: 'outBound', Data: {globalConsumers: consumer.globalStoreIp+':'+consumer.globalStorePort,Data:body}}, null]);
                       const options = {
                           method: 'POST',
